...
const bouquet=[];const bouquetStage=document.getElementById("bouquet-stage");const bouquetStatus=document.getElementById("bouquet-status");const flowerSprite=new Image();flowerSprite.src="assets/flowers-2d.webp";
...
document.getElementById("bouquet-save").addEventListener("click",async()=>{if(!bouquet.length){bouquetStatus.textContent="Pick at least one flower first, silly.";return}if(!flowerSprite.complete)await new Promise(resolve=>{flowerSprite.addEventListener("load",resolve,{once:true});flowerSprite.addEventListener("error",resolve,{once:true})});if(!flowerSprite.naturalWidth){bouquetStatus.textContent="The flowers are still waking up—try again in a second.";return}const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1080;const ctx=canvas.getContext("2d");const glow=ctx.createRadialGradient(540,500,90,540,520,720);glow.addColorStop(0,"#222b3c");glow.addColorStop(1,"#080d19");ctx.fillStyle=glow;ctx.fillRect(0,0,1080,1080);ctx.fillStyle="#e2b976";ctx.textAlign="center";ctx.font="56px serif";ctx.fillText("a bouquet for Manda",540,92);const polygon=(points,fill)=>{ctx.beginPath();ctx.moveTo(points[0][0],points[0][1]);points.slice(1).forEach(point=>ctx.lineTo(point[0],point[1]));ctx.closePath();ctx.fillStyle=fill;ctx.fill()};polygon([[270,340],[810,340],[720,895],[360,895]],"#d7b5ab");polygon([[350,320],[730,320],[685,890],[395,890]],"#f4e9dc");const sw=flowerSprite.naturalWidth/4;const sh=flowerSprite.naturalHeight/2;const count=bouquet.length;const spacing=count===1?0:Math.min(92,650/(count-1));bouquet.forEach((flowerIndex,index)=>{const dw=245,dh=430;const offset=index-(count-1)/2;const x=540+offset*spacing-dw/2;const y=145+Math.abs(offset)*10;const angle=offset*.045;const sx=(flowerIndex%4)*sw;const sy=Math.floor(flowerIndex/4)*sh;ctx.save();ctx.translate(x+dw/2,y+dh);ctx.rotate(angle);ctx.beginPath();ctx.ellipse(0,-dh/2,dw*.48,dh*.49,0,0,Math.PI*2);ctx.clip();ctx.drawImage(flowerSprite,sx,sy,sw,sh,-dw/2,-dh,dw,dh);ctx.restore()});const paper=ctx.createLinearGradient(330,570,750,920);paper.addColorStop(0,"#fff5e8");paper.addColorStop(.48,"#ead5c6");paper.addColorStop(1,"#c79a91");polygon([[250,525],[540,615],[830,525],[690,925],[390,925]],paper);ctx.strokeStyle="rgba(130,84,75,.28)";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(250,525);ctx.lineTo(540,615);ctx.lineTo(830,525);ctx.stroke();ctx.fillStyle="#9f3153";ctx.fillRect(362,790,356,70);ctx.save();ctx.translate(540,825);ctx.rotate(-.22);ctx.beginPath();ctx.ellipse(-54,0,70,32,0,0,Math.PI*2);ctx.fill();ctx.rotate(.44);ctx.beginPath();ctx.ellipse(54,0,70,32,0,0,Math.PI*2);ctx.fill();ctx.restore();ctx.beginPath();ctx.arc(540,825,34,0,Math.PI*2);ctx.fill();ctx.fillStyle="#f4eadb";ctx.font="42px serif";ctx.fillText("picked with love ♡",540,1015);const link=document.createElement("a");link.download="manda-bouquet.png";link.href=canvas.toDataURL("image/png");link.click();bouquetStatus.textContent="Wrapped and saved ♡"});
