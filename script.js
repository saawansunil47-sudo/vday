...
document.getElementById("bouquet-save").addEventListener("click",async()=>{if(!bouquet.length){bouquetStatus.textContent="Pick at least one flower first, silly.";return}const images=[flowerSprite,bouquetWrapImage];await Promise.all(images.map(image=>image.complete?Promise.resolve():new Promise(resolve=>{image.addEventListener("load",resolve,{once:true});image.addEventListener("error",resolve,{once:true})})));if(images.some(image=>!image.naturalWidth)){bouquetStatus.textContent="The bouquet is still loading—try again in a second.";return}const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1080;const ctx=canvas.getContext("2d");const glow=ctx.createRadialGradient(540,470,90,540,520,720);glow.addColorStop(0,"#222b3c");glow.addColorStop(1,"#080d19");ctx.fillStyle=glow;ctx.fillRect(0,0,1080,1080);ctx.fillStyle="#e2b976";ctx.textAlign="center";ctx.font="56px serif";ctx.fillText("a bouquet for Manda",540,92);const wrapX=250,wrapY=165,wrapW=580,wrapH=870;ctx.save();ctx.globalCompositeOperation="source-over";ctx.globalAlpha=1;ctx.drawImage(bouquetWrapImage,wrapX,wrapY,wrapW,wrapH);ctx.restore();const sw=flowerSprite.naturalWidth/4;const sh=flowerSprite.naturalHeight/2;const count=bouquet.length;const spacing=count===1?0:Math.min(78,570/(count-1));bouquet.forEach((flowerIndex,index)=>{const dw=245,dh=445;const offset=index-(count-1)/2;const x=540+offset*spacing-dw/2;const y=110+Math.abs(offset)*8;const angle=offset*.038;const sx=(flowerIndex%4)*sw;const sy=Math.floor(flowerIndex/4)*sh;ctx.save();ctx.translate(x+dw/2,y+dh);ctx.rotate(angle);ctx.beginPath();ctx.ellipse(0,-dh/2,dw*.48,dh*.49,0,0,Math.PI*2);ctx.clip();ctx.drawImage(flowerSprite,sx,sy,sw,sh,-dw/2,-dh,dw,dh);ctx.restore()});ctx.save();ctx.beginPath();ctx.rect(0,410,1080,620);ctx.clip();ctx.globalCompositeOperation="source-over";ctx.globalAlpha=1;ctx.drawImage(bouquetWrapImage,wrapX,wrapY,wrapW,wrapH);ctx.restore();ctx.fillStyle="#f4eadb";ctx.font="42px serif";ctx.fillText("picked with love ♡",540,1030);const link=document.createElement("a");link.download="manda-wrapped-bouquet.png";link.href=canvas.toDataURL("image/png");link.click();bouquetStatus.textContent="Wrapped and saved ♡"});
