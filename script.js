...
function renderBouquet(){bouquetStage.replaceChildren();if(!bouquet.length){const note=document.createElement("p");note.textContent="tap a flower to begin";bouquetStage.appendChild(note);return}bouquet.forEach(index=>{const flower=document.createElement("span");flower.className="bouquet-flower";flower.dataset.index=index;flower.setAttribute("aria-hidden","true");bouquetStage.appendChild(flower)})}
...
document.getElementById("bouquet-save").addEventListener("click",async()=>{if(!bouquet.length){bouquetStatus.textContent="Pick at least one flower first, silly.";return}if(!flowerSprite.complete)await new Promise(resolve=>{flowerSprite.addEventListener("load",resolve,{once:true});flowerSprite.addEventListener("error",resolve,{once:true})});if(!flowerSprite.naturalWidth){bouquetStatus.textContent="The flowers are still waking up—try again in a second.";return}const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1080;const ctx=canvas.getContext("2d");const glow=ctx.createRadialGradient(540,540,80,540,540,720);glow.addColorStop(0,"#222b3c");glow.addColorStop(1,"#080d19");ctx.fillStyle=glow;ctx.fillRect(0,0,1080,1080);ctx.fillStyle="#e2b976";ctx.textAlign="center";ctx.font="56px serif";ctx.fillText("a bouquet for Manda",540,105);const sw=flowerSprite.naturalWidth/4;const sh=flowerSprite.naturalHeight/2;const columns=Math.min(bouquet.length,5);const rows=Math.ceil(bouquet.length/columns);const dw=230;const dh=300;bouquet.forEach((flowerIndex,index)=>{const row=Math.floor(index/columns);const itemsInRow=row===rows-1?bouquet.length-row*columns:columns;const col=index%columns;const x=540-(itemsInRow*145)/2+col*145-dw/2+72;const y=180+row*245;const sx=(flowerIndex%4)*sw;const sy=Math.floor(flowerIndex/4)*sh;ctx.save();ctx.beginPath();ctx.ellipse(x+dw/2,y+dh/2,dw*.48,dh*.49,0,0,Math.PI*2);ctx.clip();ctx.drawImage(flowerSprite,sx,sy,sw,sh,x,y,dw,dh);ctx.restore()});ctx.fillStyle="#f4eadb";ctx.font="42px serif";ctx.fillText("picked with love ♡",540,1020);const link=document.createElement("a");link.download="manda-bouquet.png";link.href=canvas.toDataURL("image/png");link.click();bouquetStatus.textContent="Bouquet saved ♡"});
