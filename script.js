...
let pileZ=10;
document.querySelectorAll(".loose-polaroid").forEach(card=>{let drag=null;card.addEventListener("pointerdown",event=>{if(event.button!==0)return;drag={x:event.clientX,y:event.clientY,moved:false};card.setPointerCapture(event.pointerId);card.style.zIndex=String(++pileZ)});card.addEventListener("pointermove",event=>{if(!drag||!card.hasPointerCapture(event.pointerId))return;const dx=event.clientX-drag.x;const dy=event.clientY-drag.y;if(Math.hypot(dx,dy)>7)drag.moved=true;card.style.translate=dx+"px "+dy+"px"});card.addEventListener("pointerup",event=>{if(!drag)return;const wasMoved=drag.moved;drag=null;try{card.releasePointerCapture(event.pointerId)}catch(_){}if(!wasMoved){showPhoto(Number(card.dataset.photo)-1);openDialog(photoDialog)}});card.addEventListener("keydown",event=>{if(event.key!=="Enter"&&event.key!==" ")return;event.preventDefault();showPhoto(Number(card.dataset.photo)-1);openDialog(photoDialog)})});
